...
- `decks/<slug>/votes/`：html-slide-builder 簡報內嵌投票用的，該功能已整組移除、規則也已刪除。本技能的 `/polls/` 與它的差別在於 delete 允許 `isAdmin()`，講者登入後才能重置票數
...
依賴同檔案裡的 `isSignedIn()` 與 `isAdmin()` 兩個共用函式（定義見 `interactive-web-builder/firestore.rules`），以及登入憑證那條規則：
...
match /polls/{pollId}/votes/{ballot} {
  ...
  // 一般聽眾只能收回自己那票；「重置全部票數」需通過 isAdmin()（管理者登入後才有憑證）
...
- **管理密碼是整個專案共用的一組**（`config/admin`），一組密碼可以管理任何一份投票或文字雲。要換密碼跑 `interactive-web-builder/tools/set-admin-password.mjs`，把輸出的雜湊填進 `config/admin.passwordHash`。
...
- **管理密碼沒有速率限制**：規則做不到節流，任何人都能匿名登入後反覆呼叫「管理者登入」來猜密碼，而且現在一次請求就知道對錯（改成登入制的代價：以前要「寫憑證＋試刪除」兩步）。根治要把權限判定移到 Cloud Functions（需 Blaze 方案）。
- **登入狀態綁在瀏覽器的匿名 uid 上**：同一台裝置另開一個分頁載入頁面會把憑證清掉（刻意的清理，避免上次沒登出留下永久憑證），原本登入的分頁下次操作會跳「管理權限已失效」。現場只用一個分頁做管理即可。
...
- 清空：頁面上先「管理者登入」再按「重置全部票數」（只清該 `pollId` 底下的票，不影響其他投票），或在 Console 刪整個 `votes` 子集合
