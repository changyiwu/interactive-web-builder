...
name: poll-page
description: |
  產生一份可獨立部署的「即時投票網頁」：給定題目與選項，聽眾用手機開頁面投票，票數透過 Firebase Firestore 即時同步，同步畫成統計圖表（圓餅圖／甜甜圈／長條圖／累積趨勢折線圖，現場可切換）。
...
- `chart` 是**預設**圖表：`pie`｜`doughnut`｜`bar`｜`line`；頁面上四種隨時可切換
...
- **完整投票歷史**：改投票只留最後一次的時間，所以「累積趨勢」是**目前這些票何時進來的**，不是完整的變更歷史
